...
def build_targets(f0_cents: np.ndarray, voiced: np.ndarray, cutoff_hz: float = 4.0) -> np.ndarray:
    trend = lowpass_exponential(f0_cents, cutoff_hz=cutoff_hz, fps=FPS)
    snapped = snap_to_chromatic(trend)
...
def process_clip(category: str, wav_path: Path) -> Tuple[ClipMeta, Dict[str, np.ndarray]]:
    ...
    target_shift = build_targets(f0_cents, voiced)
...
def main(args: Optional[List[str]] = None) -> None:
    ...
    for category, wav_path in pairs:
        try:
            meta, arrays = process_clip(category, wav_path)
            metas.append(meta)
